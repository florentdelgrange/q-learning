"""
from https://github.com/robmsylvester/Super-Mario-Bros-DQN
"""
...
class SarstReplayMemory():
    ...
    def __init__(self, capacity, state_size, prioritize=True, priority_epsilon=0.001, priority_alpha=0.5):
        """
        Args: Capacity - int - How many samples to hold in the array. Samples all exist for exactly the same amount
        of time State_Size - numpy array shape tuple -  specifying the dimensions of the input prioritize - Boolean,
        whether to use the prioritized sum tree or to just randomly pick priority epsilon - float - small number. the
        smoothing param to add to the reward priority calculation if priortize is true priority alpha - float -
        between 0-1, the exponent used in the priority calculatiopn if prioritize is true
        """
    ...
    def append(self, state, action, reward, next_state, next_state_is_terminal):
        """
    ...
    def sample(self, batch_size):
        ...
        if self.prioritize:
            while len(chosen_sarst_indexes) < batch_size:
                ...
                # print("this chosen reward should read %.4f\n" % (abs(self.rewards[chosen_idx]) +
                # self.priority_epsilon)**self.priority_alpha)
